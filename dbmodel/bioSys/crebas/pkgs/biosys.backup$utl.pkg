CREATE OR REPLACE PACKAGE backup$utl AS
...
END;
/
CREATE OR REPLACE PACKAGE BODY backup$utl AS
  ...
  is
    ...
    l_dp_handle       NUMBER;
    l_jstate varchar2(100);
    tbl_src_tss ai$utl.T_VARCHAR_TBL;
    tbl_trgt_tss ai$utl.T_VARCHAR_TBL;
  begin
    ...
    select * bulk collect into tbl_src_tss
      from table(ai$utl.split_str(p_from_tss,','));
    select * bulk collect into tbl_trgt_tss
      from table(ai$utl.split_str(p_to_tss,','));
